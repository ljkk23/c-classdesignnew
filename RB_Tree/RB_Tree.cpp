 ...
 */
template <class T>
string RB_Tree<T>::floor_show() {
    cout << "-------------floor--------------\n";
    string str;
    queue<RB_Node<Kvalue>*> q, p;       // q 为奇数层节点， p 为偶数层节点。（从 1 开始数层数）
    q.push(this->root);
    while (!q.empty() || !p.empty()) {
        while (!q.empty()) {        // 将偶数层的节点放入 p 中
            RB_Node<Kvalue> *x = q.front();      // 取出队首节点
            q.pop();
            if (x != nullptr) {     // 如果 x 不为空，输出内容
               
                str+= "(" + x->value.toString() + ", " + (x->color ? "black" : "red") + ") ";
        ...
        while (!p.empty()) {        // 将奇数层的节点放入 q 中
            RB_Node<Kvalue> *x = p.front();      // 取出队首节点
            p.pop();
            if (x != nullptr) {
                str+= "(" + x->value.toString() + ", " + (x->color ? "black" : "red") + ") ";
